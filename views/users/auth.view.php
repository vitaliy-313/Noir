<?php include $_SERVER["DOCUMENT_ROOT"] . "/views/templates.auth/header.php"; ?>

<link rel="stylesheet" href="/css/auth.css">

<body>


    <img class="auth-image" src="/upload/Noir.auth.png" alt="">
    <form class="auth-form" action="/app/users/auth_check.php" method="POST">
        <h2>Авторизация</h2>
        <div class="auth-form2">
            <div class="auth-form-inp">
                <label for="phone">Введите телефон</label>
                <input id="phone" class="input-auth" name="phone" type="tel" pattern="(\s*)?(\+)?([- _():=+]?\d[- _():=+]?){10,14}(\s*)?" title="Введите номер телефона в формате +7 XXX XXX XX XX" value="<?= $_SESSION['save-info']['phone'] ?? "" ?>" required>
            </div>

            <div class="auth-form-inp">
                <label for="password">Введите пароль</label>
                <input type="password" class="input-auth" name="password" id="password">
            </div>
        </div>
        <?php if (empty($_SESSION['error'])) : ?>

            <p style="color:white;"> <?= $_SESSION["error"]["unfined"] ?? "" ?></p>

        <?php endif ?>

        <button class="btn-auth" name="btnAuth">Войти</button>
    </form>
    <p class="created-text"> Вы еще не зарегистрированы?</p>
    <a href="/views/users/create.view.php" class="created">Зарегистрироваться</a>
</body>
<!-- Подключение библиотеки jQuery -->
<script src="/assets/script/script/jquery.js"></script>
<!-- Подключение jQuery плагина Masked Input -->
<script src="/assets/script/script/jquery.maskedinput.js"></script>

<script defer src="/assets/script/logout.js"></script>
<?php include $_SERVER["DOCUMENT_ROOT"] . "/views/templates/footer.php"; ?>